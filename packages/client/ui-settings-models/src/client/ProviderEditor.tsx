    ...
    return (
      <>
        {(props.provider === 'google' || props.provider === 'google-vertex') ? (
          <div style={{ marginBottom: 16, padding: '12px 16px', background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.2)', borderRadius: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--dsh-color-text-primary, #f8fafc)', marginBottom: 2 }}>
                  Google Account (Pro / Workspace)
                </div>
                <div style={{ fontSize: 12, color: 'var(--dsh-color-text-secondary, #94a3b8)' }}>
                  {keyState?.configured
                    ? '✓ Signed in with Google. Gemini 3.7 Flash is ready to use.'
                    : 'Sign in with your Google Pro account to use Gemini models without an API key.'}
                </div>
              </div>
              <button
                type="button"
                onClick={() => {
                  window.open('/api/auth/google/start', 'google_auth_window', 'width=550,height=650,menubar=no,toolbar=no')
                  const listener = (event: MessageEvent) => {
                    if (event.data?.type === 'google-auth-success') {
                      window.removeEventListener('message', listener)
                      void api.credentials.describe({ refs: [keyRef] }).then((res) => {
                        if (res.result.ok) setKeyState(res.result.value.credentials[keyRef])
                      })
                    }
                  }
              ...
              >
                ...
                {keyState?.configured ? 'Re-authenticate with Google' : 'Sign in with Google'}
              </button>
            </div>
          </div>
        ) : null}
